...
CREATE TABLE `trades` (
    ...
    `profit` double GENERATED ALWAYS AS (
        (
...
/*!50003 TRIGGER `trg_bi_trades_attach_day` BEFORE INSERT ON `trades` FOR EACH ROW BEGIN
...
/*!50003 CREATE*/
/*!50017 DEFINER=`root`@`localhost`*/
/*!50003 TRIGGER `trg_ai_trades_recalc_day` AFTER INSERT ON `trades` FOR EACH ROW BEGIN
...
/*!50003 CREATE*/
/*!50017 DEFINER=`root`@`localhost`*/
/*!50003 TRIGGER `trg_au_trades_recalc_day` AFTER UPDATE ON `trades` FOR EACH ROW BEGIN
...
/*!50003 CREATE*/
/*!50017 DEFINER=`root`@`localhost`*/
/*!50003 TRIGGER `trg_ad_trades_recalc_day` AFTER DELETE ON `trades` FOR EACH ROW BEGIN
UPDATE tradingview.days d
SET
...
/* ----------------------------------------------------------------------
ADD days.trade_id FK to trades.id with cascading delete
Trigger uses a single-statement body (no DELIMITER needed).
Generated: 2025-10-22T22:20:39
---------------------------------------------------------------------- */

-- Ensure InnoDB for FK support (uncomment if needed)
-- ALTER TABLE days   ENGINE=InnoDB;
-- ALTER TABLE trades ENGINE=InnoDB;

-- 1) Add the column explicitly (no IF NOT EXISTS to maximize compatibility)
--    Make sure the type matches trades.id (adjust to BIGINT if needed).
ALTER TABLE days ADD COLUMN trade_id INT NULL;

-- 2) Optional index (explicit, though MySQL may auto-create one for the FK)
CREATE INDEX idx_days_trade_id ON days (trade_id);

-- 3) Add the foreign key (cascade deletes Days rows when their Trade is deleted)
ALTER TABLE days
ADD CONSTRAINT fk_days_trade FOREIGN KEY (trade_id) REFERENCES trades (id) ON DELETE CASCADE ON UPDATE CASCADE;

-- 4) Trigger to auto-fill days.trade_id when a trade is inserted.
--    Single-statement trigger requires no custom delimiter.
...
FOR EACH ROW
  UPDATE days
    SET trade_id = NEW.id
  WHERE id = NEW.day_id
    AND NEW.day_id IS NOT NULL;

-- 5) Optional backfill for existing data (commented out).
/*
UPDATE days d
